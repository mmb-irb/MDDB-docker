...
def deploy_stack(rm):
    ...
    subprocess.run("printenv", shell=True, check=True)
    ...
    subprocess.run("podman run -d --name cronjobs -v ${VRE_LITE_VOLUME_PATH}:/vre_lite:Z --cpus \"${CRONJOBS_CPU_LIMIT}\" --memory \"${CRONJOBS_MEMORY_LIMIT}\" --network minio_network cronjobs_image", shell=True, check=True)

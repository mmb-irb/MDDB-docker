...
def deploy_stack(rm):
    ...
    subprocess.run("podman build -t vre_lite_image --build-arg MINIO_ROOT_USER=${MINIO_ROOT_USER} --build-arg MINIO_ROOT_PASSWORD=${MINIO_ROOT_PASSWORD} --build-arg MINIO_API_PORT=${MINIO_API_INNER_PORT} --build-arg VRE_LITE_INNER_PORT=${VRE_LITE_INNER_PORT} --build-arg VRE_LITE_BASE_URL_DEVELOPMENT=${VRE_LITE_BASE_URL_DEVELOPMENT} --build-arg VRE_LITE_BASE_URL_STAGING=${VRE_LITE_BASE_URL_STAGING} --build-arg VRE_LITE_BASE_URL_PRODUCTION=${VRE_LITE_BASE_URL_PRODUCTION} --build-arg VRE_LITE_LOG_PATH=${VRE_LITE_LOG_PATH} --build-arg VRE_LITE_MAX_FILE_SIZE=${VRE_LITE_MAX_FILE_SIZE} --build-arg VRE_LITE_TIME_DIFF=${CRONJOB_VRE_LITE_TIME_DIFF} --build-arg MINIO_PROTOCOL=${MINIO_PROTOCOL} --build-arg MINIO_URL=${MINIO_URL} --build-arg MINIO_PORT=${APACHE_MINIO_OUTER_PORT} --build-arg NODE_NAME=${NODE} ./vre_lite", shell=True, check=True)
    ...
    subprocess.run("podman build -t cronjobs_image --build-arg VRE_LITE_LOG_PATH=${VRE_LITE_LOG_PATH} --build-arg VRE_LITE_TIME_DIFF=${CRONJOB_VRE_LITE_TIME_DIFF} --build-arg CJ_VRE_LITE_LOG_PATH=${CJ_VRE_LITE_LOG_PATH} --build-arg MINIO_ROOT_USER=${MINIO_ROOT_USER} --build-arg MINIO_ROOT_PASSWORD=${VRE_LITE_LOG_PATH} --build-arg MINIO_PORT=${MINIO_API_INNER_PORT} ./cronjobs", shell=True, check=True)

    subprocess.run("podman build -t workflow_image --build-arg MINIO_ROOT_USER=${MINIO_ROOT_USER} --build-arg MINIO_ROOT_PASSWORD=${MINIO_ROOT_PASSWORD} --build-arg MINIO_API_PORT=${MINIO_API_INNER_PORT} ./workflow", shell=True, check=True)
    ...
    subprocess.run("podman run -d --name minio -e MINIO_ROOT_USER=${MINIO_ROOT_USER} -e MINIO_ROOT_PASSWORD=${MINIO_ROOT_PASSWORD} -e MINIO_BROWSER_REDIRECT_URL=${MINIO_BROWSER_REDIRECT_URL} -p ${MINIO_API_OUTER_PORT}:${MINIO_API_INNER_PORT} -p ${MINIO_UI_INNER_PORT}:${MINIO_UI_INNER_PORT} -v ${MINIO_VOLUME_PATH1}:/mnt/disk1 -v ${MINIO_VOLUME_PATH2}:/mnt/disk2 -v ${MINIO_VOLUME_PATH3}:/mnt/disk3 -v ${MINIO_VOLUME_PATH4}:/mnt/disk4 --cpus \"${MINIO_CPU_LIMIT}\" --memory  \"${MINIO_MEMORY_LIMIT} \" --network minio_network --network web_network --hostname minio --healthcheck-command  \"curl -f http://localhost:${MINIO_API_INNER_PORT}/minio/health/live \" --healthcheck-interval 30s --healthcheck-timeout 10s --healthcheck-retries 5 docker.io/minio/minio:latest server --address  \":${MINIO_API_INNER_PORT} \" --console-address  \":${MINIO_UI_INNER_PORT}\" http://minio/mnt/disk{1...4}", shell=True, check=True)

    subprocess.run("podman run -d --name vre_lite -p ${VRE_LITE_OUTER_PORT}:${VRE_LITE_INNER_PORT} -v ${VRE_LITE_VOLUME_PATH}:/vre_lite --cpus \"${MINIO_CPU_LIMIT}\" --memory \"${MINIO_MEMORY_LIMIT}\" --network minio_network --network web_network vre_lite_image", shell=True, check=True)

    subprocess.run("podman run -d --name apache -p ${APACHE_HTTP_OUTER_PORT}:${APACHE_HTTP_INNER_PORT} -p ${APACHE_HTTPS_OUTER_PORT}:${APACHE_HTTPS_INNER_PORT} -p ${APACHE_MINIO_OUTER_PORT}:${APACHE_MINIO_INNER_PORT} -v ${APACHE_CERTS_VOLUME_PATH}:/usr/local/apache2/conf/ssl --cpus \"${APACHE_CPU_LIMIT}\" --memory \"${APACHE_MEMORY_LIMIT}\" --network web_network apache_image", shell=True, check=True)

    subprocess.run("podman run -d --name cronjobs -v ${VRE_LITE_VOLUME_PATH}:/vre_lite --cpus \"${CRONJOBS_CPU_LIMIT}\" --memory \"${CRONJOBS_MEMORY_LIMIT}\" --network minio_network cronjobs_image", shell=True, check=True)
